...
- `IPMSettings(tol_gap_abs, tol_feas, direct_solve_method, warm_start_no_retry, ...)` - tolerances default 1e-8. `direct_solve_method`: `'auto'` (default — CPU: single-threaded faer; CUDA: Riccati for block-tridiagonal structure e.g. MPC/LQR, Woodbury for diagonal P + low-rank A, otherwise cuDSS; benchmarked when `auto_tune=True`), `'qdldl'` (CPU), `'faer'` (CPU), `'riccati'` (CUDA, block-tridiagonal only), `'cudss'` (CUDA), `'woodbury'` (CUDA, diagonal P + low-rank A, portfolio-type)
