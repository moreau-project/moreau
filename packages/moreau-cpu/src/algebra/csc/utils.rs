    ...
    T: Num + Copy,
{
    ...
    /// Populate a column with structural zeros at scattered rows. Used for
    /// direct-x sparse cone expansions (e.g. SOC rank-2) where the u/v
    /// columns have entries at the direct-x cone's scattered `x`-indices
    /// rather than contiguous rows. `rows_sorted` MUST be strictly
    /// ascending — CSC requires column entries to be row-sorted.
    /// Writes into `vtoKKT[i]` the nzval index of the entry at row
    /// `rows_sorted[i]`.
    pub(crate) fn fill_colvec_scattered(
        &mut self,
        vtoKKT: &mut [usize],
        rows_sorted: &[usize],
        initcol: usize,
    ) {
        assert_eq!(vtoKKT.len(), rows_sorted.len());
        for (i, &row) in rows_sorted.iter().enumerate() {
            if i > 0 {
                debug_assert!(rows_sorted[i] > rows_sorted[i - 1]);
            }
            let dest = self.colptr[initcol];
    ...
    /// Populate a row with structural zeros at scattered columns. Used for
    /// direct-x sparse expansion in `Tril` form. `cols` need not be sorted;
    /// each column gets one entry at row `initrow`.
    pub(crate) fn fill_rowvec_scattered(
        &mut self,
        vtoKKT: &mut [usize],
        initrow: usize,
        cols: &[usize],
    ) {
        assert_eq!(vtoKKT.len(), cols.len());
