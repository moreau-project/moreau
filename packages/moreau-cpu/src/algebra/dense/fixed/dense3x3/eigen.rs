...
fn eigvecs_analytic<T: FloatT>(
    ...
    // and we store them in V, reordered so that V[:, i] corresponds
    // to eigenvalue w[i] (ascending order, matching eigvals_analytic).
    //
    // The vectors v1, v2, v3 were computed for eigenvalues
    // w[sidx[0]], w[sidx[1]], w[sidx[2]] respectively (sorted by
    // descending absolute value). We need to invert the permutation
    // so that V[:, j] = the eigenvector for w[j].
    if let Some(vecs) = V {
        let evecs = [v1, v2, v3];
        // inv_perm[j] = k means eigenvector evecs[k] belongs to w[j]
        let mut inv_perm = [0usize; 3];
        for (k, &j) in sidx.iter().enumerate() {
            inv_perm[j] = k;
        }
        for j in 0..3 {
            vecs.data[j * 3..(j + 1) * 3].copy_from_slice(&evecs[inv_perm[j]]);
        }
    }
