    ...
    T: FloatT,
{
    ...
    /// Replace the diagonal sign vector used for sign-aware regularisation.
    /// Re-permutes `signs` through the AMD ordering captured at construction.
    /// Length must match the matrix dimension.
    pub fn update_dsigns(&mut self, signs: &[i8]) {
        assert_eq!(signs.len(), self.workspace.Dsigns.len());
        permute(&mut self.workspace.Dsigns, signs, &self.perm);
    }
...
fn _factor_inner<T: FloatT>(
    ...
    *regularize_count = 0;
    ...
    if !logical_factor {
        ...
        D[0] = Ax[0];
        if regularize_enable && D[0].abs() < regularize_eps {
            // Regularise only when the pivot is genuinely near-singular.
            // The textbook `D[k] * sign < eps` form additionally fires
            // on large pivots whose sign disagrees with the prescribed
            // dsigns — a regular occurrence on AMD-ordered factors of
            // quasidefinite matrices that include rank-1 sparse expansion
            // columns (e.g. GenPow's rank-9 PD axes), where the per-pivot
            // sign isn't structurally pinned. Clamping those legitimate
            // wrong-sign pivots to `±delta` wrecks the factor.
    ...
    for k in 1..n {
        ...
        if !logical_factor {
            // apply dynamic regularization (see comment above the k=0
            // branch — only clamp truly near-singular pivots).
            if regularize_enable && D[k].abs() < regularize_eps {
...
fn _solve<T: FloatT>(Lp: &[usize], Li: &[usize], Lx: &[T], Dinv: &[T], b: &mut [T]) {
    // Runtime bounds validation - these checks run in release mode to prevent UB
    let n = b.len();
    assert!(
        Lp.len() > n,
        "QDLDL solve: Lp array length {} too short for matrix dimension {}",
        Lp.len(),
        n
    );
    assert!(
        Dinv.len() >= n,
        "QDLDL solve: Dinv array length {} too short for dimension {}",
        Dinv.len(),
        n
    );
    if n > 0 {
        ...
        assert!(
            nnz <= Li.len(),
            "QDLDL solve: Li array length {} too short for {} nonzeros",
            Li.len(),
            nnz
        );
        assert!(
            nnz <= Lx.len(),
            "QDLDL solve: Lx array length {} too short for {} nonzeros",
            Lx.len(),
            nnz
        );
    }
...
pub(crate) fn permute<T: Copy>(x: &mut [T], b: &[T], p: &[usize]) {
    // Runtime bounds validation (release mode safe)
    assert!(
        x.len() >= p.len(),
        "permute: output array length {} too short for permutation of length {}",
        x.len(),
        p.len()
    );
    assert!(
        b.len() >= p.len(),
        "permute: input array length {} too short for permutation of length {}",
        b.len(),
        p.len()
    );
    if !p.is_empty() {
        let max_idx = *p.iter().max().unwrap();
        assert!(
            max_idx < b.len(),
            "permute: permutation index {} out of bounds for array of length {}",
            max_idx,
            b.len()
        );
    }
...
pub(crate) fn ipermute<T: Copy>(x: &mut [T], b: &[T], p: &[usize]) {
    ...
    assert!(
        x.len() >= p.len(),
        "ipermute: output array length {} too short for permutation of length {}",
        x.len(),
        p.len()
    );
    assert!(
        b.len() >= p.len(),
        "ipermute: input array length {} too short for permutation of length {}",
        b.len(),
        p.len()
    );
    if !p.is_empty() {
        ...
        assert!(
            max_idx < x.len(),
            "ipermute: permutation index {} out of bounds for array of length {}",
            max_idx,
            x.len()
        );
    }
