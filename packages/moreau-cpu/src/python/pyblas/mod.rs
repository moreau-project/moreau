...
// Re-export pointer types for pre-validation in module init (#189).
pub use blas_types::PyBlasPointers;
pub use lapack_types::PyLapackPointers;
...
macro_rules! get_ptr {
    ($api: ident, $str: tt) => {{
        let ptr = get_capsule_void_ptr(&$api, $str)?;
        if ptr.is_null() {
            return Err(pyo3::exceptions::PyRuntimeError::new_err(format!(
                "BLAS/LAPACK function '{}' returned null pointer",
                $str
            )));
        }
