    ...
    T: FloatT,
{
    ...
    fn is_linear_only(&self) -> bool {
        self.type_counts.keys().all(|k| {
            matches!(
                k,
                SupportedConeTag::ZeroCone | SupportedConeTag::NonnegativeCone
            )
        })
    }

    fn is_sparse_expandable(&self) -> bool {
        // composite cones are never queried for sparse-expandability directly
        unreachable!();
    }

    fn supports_smoothed(&self) -> bool {
        let n_zero = self.get_type_count(SupportedConeTag::ZeroCone);
        let n_nonneg = self.get_type_count(SupportedConeTag::NonnegativeCone);
        let n_soc = self.get_type_count(SupportedConeTag::SecondOrderCone);
        n_zero + n_nonneg + n_soc == self.cones.len()
    }
    ...
    fn smoothing(&mut self, z: &mut [T], s: &[T], work: &[T], μ: T) {
        for (cone, rng) in zip(&mut self.cones, &self.rng_cones) {
            let zi = &mut z[rng.clone()];
            let si = &s[rng.clone()];
            let worki = &work[rng.clone()];
            cone.smoothing(zi, si, worki, μ);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_is_linear_only_zero_and_nonneg() {
        let cones = CompositeCone::<f64>::new(&[
            SupportedConeT::ZeroConeT(3),
            SupportedConeT::NonnegativeConeT(5),
        ]);
        assert!(cones.is_linear_only());
    }

    #[test]
    fn test_is_linear_only_zero_only() {
        let cones = CompositeCone::<f64>::new(&[SupportedConeT::ZeroConeT(4)]);
        assert!(cones.is_linear_only());
    }

    #[test]
    fn test_is_linear_only_nonneg_only() {
        let cones = CompositeCone::<f64>::new(&[SupportedConeT::NonnegativeConeT(4)]);
        assert!(cones.is_linear_only());
    }

    #[test]
    fn test_is_linear_only_empty() {
        let cones = CompositeCone::<f64>::new(&[]);
        assert!(cones.is_linear_only());
    }

    #[test]
    fn test_is_linear_only_false_with_soc() {
        let cones = CompositeCone::<f64>::new(&[
    ...
    #[test]
    fn test_is_linear_only_false_with_exp() {
        let cones = CompositeCone::<f64>::new(&[
            SupportedConeT::NonnegativeConeT(3),
    ...
    #[test]
    fn test_is_linear_only_false_with_pow() {
        let cones = CompositeCone::<f64>::new(&[
            SupportedConeT::NonnegativeConeT(3),
    ...
    #[test]
    fn test_is_linear_only_false_soc_only() {
        let cones = CompositeCone::<f64>::new(&[SupportedConeT::SecondOrderConeT(3)]);
        assert!(!cones.is_linear_only());
    }
}
