    ...
    T: FloatT,
{
    ...
    fn smoothing(&mut self, z: &mut [T], _s: &[T], work: &[T], μ: T) {
        // A rounded boundary point may pass strict feasibility while its
        // barrier Hessian overflows. Start Newton safely inside the dual cone.
        z[0] -= μ * (1.051_383_945_322_714).as_T();
        z[1] += μ * (0.556_409_619_469_370).as_T();
        z[2] += μ * (1.258_967_884_768_947).as_T();
