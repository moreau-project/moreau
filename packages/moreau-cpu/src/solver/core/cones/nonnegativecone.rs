    ...
    T: FloatT,
{
    ...
    fn step_length(
        ...
        for i in 0..z.len() {
            if dz[i] < T::zero() {
                let candidate = -z[i] / dz[i];
                αz = T::min(αz, candidate);
            }
            if ds[i] < T::zero() {
                let candidate = -s[i] / ds[i];
                αs = T::min(αs, candidate);
            }
        }
    ...
    fn smoothing(&mut self, z: &mut [T], _s: &[T], work: &[T], μ: T) {
        // For nonnegative cone: z_new = (w + sqrt(w² + 4μ))/2
        // where w = z - s. This ensures z > 0 and s = z - w > 0.
        let four: T = (4.0).as_T();
        let two: T = (2.0).as_T();
        let four_mu = four * μ;
        for (zi, &wi) in zip(z.iter_mut(), work) {
            *zi = (wi + T::sqrt(wi * wi + four_mu)) / two;
        }
    }
}
