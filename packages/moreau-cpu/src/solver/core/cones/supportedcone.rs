...
impl<T> std::fmt::Debug for SupportedConeT<T> {
...
fn test_cone_ranges() {
    let cones = [
        ...
        SupportedConeT::SecondOrderConeT::<f64>(3),
    ];

    let rngs: Vec<std::ops::Range<usize>> = vec![0..3, 3..3, 3..6];
...
mod tests {
    ...
    // Note: test_new_collapsed_soc_to_nonnegative was testing SOC(1) → NonnegativeCone
    // collapsing logic. Since SOC dim=1 is no longer valid, this test is removed.
    // The collapsing logic still exists but is now unreachable in normal operation.
    ...
    fn test_new_collapsed_mixed() {
        // Test mixed cones with nonnegatives that get consolidated
        let cones = vec![
