...
{
    pub fn new(dim: usize) -> Self {
        Self::new_with_sparse_expansion(dim, true)
    }

    /// Construct an SOC, optionally forcing the dense-Hs form regardless
    /// of cone dimension. Direct-x SOC cones must use the dense form —
    /// the sparse rank-2 expansion adds extra columns/rows to the KKT
    /// matrix that aren't plumbed through the direct-x (1,1) assembly.
    pub fn new_with_sparse_expansion(dim: usize, allow_sparse: bool) -> Self {
        ...
        let sparse_data = {
            if allow_sparse && dim > SOC_NO_EXPANSION_MAX_SIZE {
    ...
    T: FloatT,
{
    ...
    fn mul_Hs(&mut self, y: &mut [T], x: &[T], _work: &mut [T]) {
        // y = WᵀW x = η²·(2(wᵀx)w + Jx), computed directly (work buffer unused)
    ...
    fn smoothing(&mut self, z: &mut [T], _s: &[T], work: &[T], μ: T) {
        // SOC smoothing: project w = z - s to cone interior while preserving w
        // Analytic formula that ensures z₀² - ||z₁:||² = μ
        let two: T = (2.0).as_T();
        let four: T = (4.0).as_T();

        let w0sq = work[0] * work[0];
        let w1sq = work[1..].sumsq();
        // Avoid catastrophic cancellation: w0² - ||w1||² = (w0 - ||w1||)(w0 + ||w1||)
        let w1norm = T::sqrt(w1sq);
        let res = (work[0] - w1norm) * (work[0] + w1norm);

        let c = res / μ;
        let inner = c * c + four * (two * (w0sq + w1sq) / μ + four);
        let γ = (c + T::sqrt(inner)) / two;
        let γsq_minus_4 = γ * γ - four;

        // Choose root based on sign of w[0]
        let ρ = if work[0] > T::zero() {
            (γ + T::sqrt(γsq_minus_4)) / two
        } else {
            (γ - T::sqrt(γsq_minus_4)) / two
        };

        // Compute z[0]
        let ρ_minus_1 = ρ - T::one();
        if T::abs(ρ_minus_1) < T::epsilon() {
            z[0] = T::sqrt(μ + w1sq / four);
        } else {
            z[0] = ρ / ρ_minus_1 * work[0];
        }

        // Compute z[1:]
        let d = ρ / (ρ + T::one());
        for (zi, &wi) in z[1..].iter_mut().zip(&work[1..]) {
            *zi = d * wi;
        }
    }
}
...
// other functions without running into borrow-checker conflicts
    ...
    T: FloatT,
{
    ...
    // Due to T::max above, c should never be negative. But if numerical
    // instability produces this state, return a small step length rather
    // than panicking. This allows the solver to recover gracefully.
    if c < T::zero() {
        return T::epsilon().sqrt();
    }
