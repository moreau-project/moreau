    ...
    T: FloatT,
{
    pub fn new(
        ...
        let mut opts = QDLDLSettingsBuilder::default()
            ...
            .regularize_eps(settings.ipm.dynamic_regularization_eps)
            .regularize_delta(settings.ipm.dynamic_regularization_delta)
            .amd_dense_scale(1.5)
            .build()
            .unwrap();
        opts.perm = perm;
    ...
    T: FloatT,
{
    ...
    fn update_dsigns(&mut self, signs: &[i8]) {
        self.factors.update_dsigns(signs);
    }
