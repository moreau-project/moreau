...
where
    T: FloatT,
{
    fn to_faer(&self, _Dsigns: &'a [i8]) -> LdltRegularization<'a, T> {
        // Pass `None` for dynamic_regularization_signs so faer regularises
        // only genuinely near-singular pivots (`|D[k]| < eps` clamped to
        // `±delta` with `D[k]`'s natural sign). The sign-aware variant
        // (`Some(signs)`) also clamps *large* pivots whose sign disagrees
        // with the prescribed signs — a regular occurrence on AMD-ordered
        // factors of quasidefinite matrices that include rank-1 sparse
        // expansion columns (GenPow's rank-9 PD axes), where the per-pivot
        // sign isn't structurally pinned. Clamping those legitimate
        // wrong-sign pivots to `±delta` (~2e-7) wrecks the factor.
        let regularize = self.regularize;
        LdltRegularization {
            dynamic_regularization_signs: None,
            dynamic_regularization_delta: if regularize { self.delta } else { T::zero() },
            dynamic_regularization_epsilon: if regularize { self.eps } else { T::zero() },
        }
    }
}
    ...
    T: FloatT,
{
    pub fn new(
        ...
        // Determine parallelism based on solver mode and max_threads.
        // "faer-1t" always uses single-threaded (Par::Seq).
        // "faer-nt" uses max_threads (0 = auto, 1 = seq, N = N threads).
        // "auto" and "faer" default to single-threaded.
        let parallelism = {
            let method = settings.ipm.direct_solve_method.as_str();
            match method {
                "faer-nt" => match settings.ipm.max_threads {
                    0 => Par::rayon(0),
                    1 => Par::Seq,
                    _ => Par::rayon(settings.ipm.max_threads as usize),
                },
                // "auto", "faer", "faer-1t" all use single-threaded
                _ => Par::Seq,
            }
        };
        ...
        let regularizer_params = FaerLDLRegularizerParams {
            regularize: settings.ipm.dynamic_regularization_enable,
            delta: settings.ipm.dynamic_regularization_delta,
            eps: settings.ipm.dynamic_regularization_eps,
        };
    ...
    T: FloatT,
{
    ...
    fn offset_values(&mut self, index: &[usize], offset: T, signs: &[i8]) {
        // this is replicating the offset_values function in qdldl
        let nzval = &mut self.perm_kkt.nzval; // post perm internal data
        let AtoPAPt = &self.perm_map; //mapping from input matrix entries
    ...
    fn update_dsigns(&mut self, signs: &[i8]) {
        assert_eq!(signs.len(), self.perm_dsigns.len());
        permute(&mut self.perm_dsigns, signs, &self.perm);
    }
    ...
    fn refactor(&mut self, _kkt: &CscMatrix<T>) -> bool {
        ...
        self.symbolic_cholesky
            ...
            .is_ok() // convert to bool for consistency with qdldl.   Should really return Result here and elsewhere
    }
...
fn test_faer_qp() {
    ...
    let mut settings = crate::solver::DefaultSettings::default();
    settings.ipm.direct_solve_method = "faer".to_string();
