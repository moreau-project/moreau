    ...
    P: PardisoInterface + PardisoCustomInitialize,
{
    ...
    fn initialize(
        ...
        for (i, &iparm) in settings.ipm.pardiso_iparm.iter().enumerate() {
            if iparm != i32::MIN {
                ps.set_iparm(i, iparm);
            }
        }

        if settings.ipm.pardiso_verbose {
...
impl MKLPardisoDirectLDLSolver {
    pub fn new(
        ...
        let nthreads = settings.ipm.max_threads as i32;
    ...
    P: PardisoInterface + PardisoCustomInitialize,
{
    ...
    fn update_dsigns(&mut self, _signs: &[i8]) {
        //no-op.  Pardiso does not consume the dsigns vector.
    }
    ...
    fn refactor(&mut self, kkt: &CscMatrix<f64>) -> bool {
        ...
        // would be preferable to propogate Errors here
