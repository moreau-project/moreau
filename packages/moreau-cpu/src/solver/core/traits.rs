...
use super::ffi::*;
use super::{
    cones::{CompositeXCone, Cone},
    CoreSettings, ScalingStrategy, SettingsError,
};
...
pub trait Variables<T: FloatT>: std::fmt::Debug {
...
pub trait Info<T>: InfoPrint<T> + MoreauFFI<Self> + Sized + Clone
where
    T: FloatT,
{
    ...
    /// Snapshot `variables` into `best_vars` (along with the current metric
    /// state) iff the current iterate meets the reduced tolerances.  Called
    /// once per iteration from the main loop.
    ...
    /// If the solve is terminating in an error / limit state but a reduced
    /// tolerance iterate was snapshotted during the run, restore that iterate
    /// into `variables` and promote the status to `AlmostSolved`.  Returns
    /// `true` if a restore occurred.
...
pub trait Settings<T: FloatT>: MoreauFFI<Self> + Sized + Clone {
    ...
    /// Validate settings against cone properties.
    /// Called by `update_settings` with the solver's composite cone.
    /// Default implementation accepts all settings.
    fn validate_with_cones<C: Cone<T>>(&self, _cones: &C) -> Result<(), SettingsError> {
        Ok(())
    }
}
