...
use crate::solver::implementations::default::{DiffMethod, SolverType};
...
/// FFI interface for differentiation method
#[allow(missing_docs)]
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub enum DiffMethodFFI {
    AUTO = 0,
    SMOOTHED = 1,
    EXACT = 2,
}

impl From<DiffMethodFFI> for DiffMethod {
    fn from(value: DiffMethodFFI) -> Self {
        match value {
            DiffMethodFFI::AUTO => DiffMethod::Auto,
            DiffMethodFFI::SMOOTHED => DiffMethod::Smoothed,
            DiffMethodFFI::EXACT => DiffMethod::Exact,
        }
    }
}

impl From<DiffMethod> for DiffMethodFFI {
    fn from(value: DiffMethod) -> Self {
        match value {
            DiffMethod::Auto => DiffMethodFFI::AUTO,
            DiffMethod::Smoothed => DiffMethodFFI::SMOOTHED,
            DiffMethod::Exact => DiffMethodFFI::EXACT,
        }
    }
}
...
/// FFI interface for [`SolverType`](crate::solver::implementations::default::SolverType)
#[allow(missing_docs)]
#[repr(C)]
...
impl From<SolverType> for SolverTypeFFI {
    fn from(value: SolverType) -> Self {
        match value {
            SolverType::IPM => SolverTypeFFI::IPM,
        }
    }
}

impl From<SolverTypeFFI> for SolverType {
    fn from(value: SolverTypeFFI) -> Self {
        match value {
            SolverTypeFFI::IPM => SolverType::IPM,
        }
    }
}
