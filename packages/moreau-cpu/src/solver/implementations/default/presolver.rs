...
pub(crate) struct Presolver<T> {
    // original cones; retained to anchor the `T` type parameter
    ...
    T: FloatT,
{
    ...
    fn reduce_A_b(&self, A: &CscMatrix<T>, b: &[T]) -> (CscMatrix<T>, Vec<T>) {
        let map = self
            .reduce_map
            .as_ref()
            .expect("reduce_A_b called before compute_reduce_map. This is an internal error.");
    ...
    fn reduce_cones(&self, cones: &[SupportedConeT<T>]) -> Vec<SupportedConeT<T>> {
        let map = self
            .reduce_map
            .as_ref()
            .expect("reduce_cones called before compute_reduce_map. This is an internal error.");
    ...
    pub(crate) fn reverse_presolve(
        ...
        let map = self.reduce_map.as_ref().expect(
            "reverse_presolve called before compute_reduce_map. This is an internal error.",
        );
        let mut ctr = 0;
