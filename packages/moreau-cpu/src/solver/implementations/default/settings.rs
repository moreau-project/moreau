...
// ============================================================================
// SolverType - Enum for solver algorithm selection
// ============================================================================

/// Solver algorithm type.
///
/// Currently only IPM (Interior Point Method) is supported.
...
/// Threshold for Auto resolution of DiffMethod.
/// When μ < this value, Auto selects Exact; otherwise Smoothed.
pub const AUTO_SMOOTHED_MU_THRESHOLD: f64 = 1e-6;

/// Differentiation method for backward pass.
///
/// Controls how the cone derivative H is computed in the HSDE backward KKT system.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum DiffMethod {
    /// Automatically select method based on μ at convergence.
    /// If μ < 1e-6, uses Exact; otherwise uses Smoothed with the converged μ.
    #[default]
    Auto,
    /// Use H = (I + μ ∇²φ*(z_cached))⁻¹, computed from a cached central-path
    /// iterate at μ ≈ `diff_smoothing_mu`. Produces smooth gradients.
    Smoothed,
    /// Use H = DΠ_{K*}(u), the Jacobian of the dual cone projection.
    /// Produces exact but potentially discontinuous gradients at cone boundaries.
    Exact,
}

// ============================================================================
// IPMSettings - Interior Point Method specific settings
// ============================================================================
...
pub struct IPMSettings<T: FloatT> {
    ...
    ///Options: "auto" (faer single-thread), "qdldl", "faer-1t", "faer-nt", "mkl", "panua"
    ...
    /// Differentiation method for backward pass.
    /// `Auto` (default): selects Exact or Smoothed based on μ at convergence.
    /// `Smoothed`: uses cached central-path iterate at `diff_smoothing_mu`.
    /// `Exact`: standard cone projection Jacobian.
    #[builder(default)]
    pub diff_method: DiffMethod,

    /// Smoothing parameter μ for `Smoothed` differentiation.
    /// During the forward solve, the iterate is cached when μ crosses this threshold.
    /// The cached central-path point is used for the backward pass.
    #[builder(default = "(1e-4).as_T()")]
    pub diff_smoothing_mu: T,

    /// Step factor for central-path refinement in smoothed differentiation.
    /// Each refinement iteration multiplies the current μ by this factor toward
    /// `diff_smoothing_mu`. Larger values mean fewer iterations but may cause
    /// the line search to take smaller steps.
    #[builder(default = "(30.0).as_T()")]
...
impl<T> Default for IPMSettings<T>
where
    T: FloatT,
{
    fn default() -> IPMSettings<T> {
        IPMSettingsBuilder::<T>::default().build().unwrap()
    }
}

impl From<SettingsError> for IPMSettingsBuilderError {
    fn from(e: SettingsError) -> Self {
        IPMSettingsBuilderError::ValidationError(e.to_string())
    }
}

impl<T> IPMSettingsBuilder<T>
where
    T: FloatT,
{
...
// ============================================================================
// DefaultSettings - Main solver settings with nested IPM settings
// ============================================================================

/// Standard-form solver settings implementing the [`Settings`](crate::solver::core::traits::Settings) trait.
///
/// Contains common solver settings (max_iter, time_limit, verbose, enable_grad)
/// plus nested IPM-specific settings in the `ipm` field.
#[derive(Debug, Clone)]
pub struct DefaultSettings<T: FloatT> {
    // ========================================================================
    // Common settings (shared across solver types)
    // ========================================================================
    ...
    /// Verbose printing.
    ///
    /// When `verbose=true`, the solver prints per-iteration convergence information.
    /// This is useful for debugging but has minimal performance impact on CPU.
    /// For GPU solvers, verbose mode requires CPU/GPU synchronization every iteration
    /// which can significantly impact performance - see CUDA settings documentation.
    pub verbose: bool,

    /// enable gradient computation via backward pass
    pub enable_grad: bool,

    /// YOLO mode: run exactly yolo_num_iters iterations with no convergence checking.
    /// Returns MaxIterations status for all problems.
    pub yolo: bool,
...
macro_rules! check_immutable_setting {
    ($self:expr, $prev:expr, $field:ident) => {
        if $self.ipm.$field != $prev.ipm.$field {
    ...
    T: FloatT,
{
    ...
    fn validate(&self) -> Result<(), SettingsError> {
        ...
        if !self.ipm.direct_kkt_solver {
            return Err(SettingsError::BadFieldValue("direct_kkt_solver"));
        }

        //check that the choice of LDL solver (string) is valid
        validate_direct_solve_method(&self.ipm.direct_solve_method)?;

        // check that the chordal decomposition merge method (string) is valid
        #[cfg(feature = "sdp")]
        validate_chordal_decomposition_merge_method(&self.ipm.chordal_decomposition_merge_method)?;

        // check the pardiso iparm settings.   Currently a no-op
        #[cfg(any(feature = "pardiso-mkl", feature = "pardiso-panua"))]
        validate_pardiso_iparm(&self.ipm.pardiso_iparm)?;

        // smoothed differentiation parameter validation
        if self.ipm.diff_smoothing_step_factor <= T::one() {
            return Err(SettingsError::BadFieldValue("diff_smoothing_step_factor"));
        }
        if self.ipm.diff_smoothing_mu <= T::zero() {
            return Err(SettingsError::BadFieldValue("diff_smoothing_mu"));
        }

        Ok(())
    }

    fn validate_with_cones<C: crate::solver::core::cones::Cone<T>>(
        &self,
        cones: &C,
    ) -> Result<(), SettingsError> {
        if self.ipm.diff_method == DiffMethod::Smoothed && !cones.supports_smoothed() {
            return Err(SettingsError::BadFieldValue("diff_method"));
        }
        Ok(())
    }
...
impl<T: FloatT> MoreauFFI<Self> for DefaultSettings<T> {
...
fn validate_direct_solve_method(direct_solve_method: &str) -> Result<(), SettingsError> {
    match direct_solve_method {
        ...
        "faer-1t" | "faer-nt" => Ok(()),
        // Keep "faer" as an alias for "faer-1t" for backward compatibility
        #[cfg(feature = "faer-sparse")]
        "faer" => Ok(()),
...
fn test_settings_validate() {
    ...
    let mut bad_settings = DefaultSettings::<f64>::default();
    bad_settings.ipm.direct_solve_method = "foo".to_string();
    assert!(bad_settings.validate().is_err());

    #[cfg(feature = "sdp")]
    {
        // fail on unknown chordal decomposition merge method
        let mut bad_chordal = DefaultSettings::<f64>::default();
        bad_chordal.ipm.chordal_decomposition_merge_method = "foo".to_string();
        assert!(bad_chordal.validate().is_err());
    }

    // try to overlay prohibited update values
    let mut oldsettings = DefaultSettings::<f64>::default();
    oldsettings.ipm.presolve_enable = false;

    let newsettings = DefaultSettings::<f64>::default();
    assert!(newsettings.validate_as_update(&oldsettings).is_err());

    // try to overlay allowed update values
    let mut oldsettings = DefaultSettings::<f64>::default();
    oldsettings.max_iter = 10;

    let mut newsettings = DefaultSettings::<f64>::default();
    newsettings.max_iter = 11;
    assert!(newsettings.validate_as_update(&oldsettings).is_ok());
}

#[test]
fn test_reject_riccati_direct_solve_method_on_cpu() {
    ...
    for method in ["riccati", "woodbury", "cudss"] {
        let mut s = DefaultSettings::<f64>::default();
        s.ipm.direct_solve_method = method.to_string();
        assert!(
            s.validate().is_err(),
            "CPU should reject direct_solve_method='{}'",
            method
        );
    }
}
