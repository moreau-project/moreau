...
pub(crate) use augment_compact::CompactAugmentedProblem;
    ...
    T: FloatT,
{
    ...
    pub(crate) fn decomp_augment(
        ...
        if settings.ipm.chordal_decomposition_compact {
        ...
        T: FloatT,
    {
        ...
        assert_eq!(settings.ipm.chordal_decomposition_compact, self.H.is_none());
        assert_eq!(
            settings.ipm.chordal_decomposition_compact,
        ...
        // Direct-x dual `z_x` lives outside chordal-augmented rows (it
        // pairs with `x[J]`, which is preserved by chordal at indices < n).
        // Carry it over verbatim; chordal+direct-x-PSD is gated upstream
        // so in practice this is just zero-length, but it keeps the
        // invariant that `new_vars.z_x` mirrors `old_vars.z_x`.
        if !old_vars.z_x.is_empty() {
            new_vars.z_x = old_vars.z_x.clone();
        }
        ...
        if settings.ipm.chordal_decomposition_complete_dual {
