    ...
    T: FloatT,
{
    pub(crate) fn new(
        ...
        chordal_info.find_sparsity_patterns(
            ...
            b,
            cones,
            settings.ipm.chordal_decomposition_merge_method.as_str(),
        );
    ...
    /// Construct ChordalInfo from sparsity pattern only (for batch mode).
    ///
    /// Uses a zero b vector so that the sparsity mask is driven purely by
    /// the A matrix structure. This correctly identifies decomposable PSD cones
    /// regardless of per-problem b values, since b only affects numerical values,
    /// not the sparsity pattern.
    /// Create ChordalInfo from sparsity pattern only (no b values available).
    /// Uses a conservative b mask: any row that has all-zero A entries is
    /// assumed to have nonzero b (i.e., structurally dense). This ensures
    /// chordal decomposition doesn't drop coupling entries that appear only in b.
    /// Create ChordalInfo using a boolean mask for b's nonzero pattern.
    ///
    /// The compiled solver may not have b values at construction time,
    /// but the caller can provide a mask indicating which entries of b
    /// are structurally nonzero. This is critical for correct chordal
    /// decomposition: entries that appear only in b (not in A) must be
    /// included in the sparsity pattern, otherwise cliques lose coupling
    /// and the solver can't recover implicitly-determined variables.
    ///
    /// If no mask is provided (None), falls back to a conservative estimate
    /// that marks all-zero-A rows as having nonzero b.
    pub(crate) fn new_from_pattern(
        ...
        let mask: Vec<bool>;
        let b_mask = match b_sparsity_pattern {
            Some(m) => m,
            None => {
                // Conservative fallback: assume nonzero b where A row is all-zero
                let mut row_has_nz = vec![false; m];
                for &row_idx in A.rowval.iter() {
                    row_has_nz[row_idx] = true;
                }
                mask = row_has_nz.iter().map(|&has_nz| !has_nz).collect();
                &mask
            }
        };
        let mut b_pattern = vec![T::zero(); m];
        for (i, &is_nz) in b_mask.iter().enumerate() {
            if is_nz {
                b_pattern[i] = T::one();
            }
        }
        Self::new(A, &b_pattern, cones, settings)
    }
