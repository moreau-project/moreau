...
def test_compiled_direct_cone_warm_start(batch_size):
    ...
    solver.setup([4., 0.25], [])
    q, b = np.tile([-4., 2.], (batch_size, 1)), np.empty((batch_size, 0))
    ...
    assert np.all(np.asarray(solver.info.iterations) == 0)
    np.testing.assert_allclose(warm.x, np.tile([1., 0.], (batch_size, 1)), atol=1e-6)
    np.testing.assert_allclose(warm.z_x, np.tile([0., 2.], (batch_size, 1)), atol=1e-6)
